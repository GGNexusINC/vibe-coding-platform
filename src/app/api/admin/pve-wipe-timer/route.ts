...
// GET — public, returns current PvE wipe timer
export async function GET() {
  const sb = getSupabase();
  const { data } = await sb
    .from("site_settings")
    .select("value")
    .eq("key", "pve_wipe_timer")
    .single();
...
// POST — admin only, sets PvE wipe timer
export async function POST(req: Request) {
  const admin = await getAdminSession();
  if (!admin) return NextResponse.json({ ok: false, error: "Unauthorized." }, { status: 401 });

  const body = await req.json().catch(() => ({}));
  const rawWipeAt: string | null = body?.wipeAt ?? null;
  const label: string = String(body?.label ?? "PvE Season Reset").trim().slice(0, 80);
  ...
  const { error } = await sb.from("site_settings").upsert(
    { key: "pve_wipe_timer", value: { wipeAt, label, setAt: new Date().toISOString() } },
    { onConflict: "key" }
  );

  if (error) {
    console.error("[pve-wipe-timer] Supabase error:", JSON.stringify(error));
