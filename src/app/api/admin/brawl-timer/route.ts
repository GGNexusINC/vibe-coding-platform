...
// GET — public, returns current brawl timer
...
// POST — admin only, sets brawl timer
export async function POST(req: Request) {
  ...
  if (wipeAt === undefined) {
    return NextResponse.json({ ok: false, error: "Invalid brawl date/time." }, { status: 400 });
  }

  const sb = getSupabase();
  const { error } = await sb.from("site_settings").upsert(
    { key: "brawl_timer", value: { wipeAt, label, setAt: new Date().toISOString() } },
    { onConflict: "key" }
  );

  if (error) {
    console.error("[brawl-timer] Supabase error:", JSON.stringify(error));
